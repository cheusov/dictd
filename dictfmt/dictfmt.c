...
static void increase_fmt_hwcount(void)
{
	if (!quiet_mode){
...
static void update_alphabet(const char *word)
{
	...
	while (*p){
		len = utf8_mode ? mbrlen(p, MB_CUR_MAX, &ps) : 1;
		assert((int) len >= 0);
...
static void fmt_predefined_headwords_before (void)
{
	if (type == INDEXONLY)
		return;
...
static void fmt_predefined_headwords_after(void)
{
	if (type == INDEXONLY)
		return;
